...
static THD_FUNCTION(CaptureImage, arg) {
	...
	dcmi_prepare();
	 ...
	 while(1){
		    	...
		    	chBSemSignal(&img_center_ready_sem);

		    	chBSemWait(&img_capture);
				...
				wait_image_ready();
				//signals an image has been captured
				chBSemSignal(&img_bottom_ready_sem);
		    }
		}
...
static THD_FUNCTION(ProcessImage, arg) {
    ...
    uint8_t *img_ctr_buff_ptr;
        ...
        while(1){
        	...
        	chBSemWait(&img_center_ready_sem);
    		...
    		//movement = green_light(red_ctr, green_ctr, blue_ctr, movement);

    		if(movement)
    		{
    			//send a signal to capture
    			chBSemSignal(&img_capture);
    	    	//waits until an image has been captured
    	        chBSemWait(&img_bottom_ready_sem);
    			//gets the pointer to the array filled with the last image in RGB565
    			img_bot_buff_ptr = dcmi_get_last_image_ptr();

    			//Extracts the different colors of the pixels.
    			for(uint16_t i = 0 ; i < (2 * IMAGE_BUFFER_SIZE) ; i+=2){
    	        	clr_intensity[i/2] = (uint8_t)img_bot_buff_ptr[i]&0xF8;
